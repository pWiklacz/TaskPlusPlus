...
internal sealed class ChangeTaskEnergyCommandHandler : ICommandHandler<ChangeTaskEnergyCommand>
{
    private readonly IUnitOfWork _unitOfWork;

    public ChangeTaskEnergyCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }
    public async Task<Result> Handle(ChangeTaskEnergyCommand request, CancellationToken cancellationToken)
    {
        var validator = new TaskEnergyDtoValidator();
