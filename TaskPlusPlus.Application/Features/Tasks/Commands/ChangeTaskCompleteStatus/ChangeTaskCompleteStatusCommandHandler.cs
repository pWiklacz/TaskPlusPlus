...
internal sealed class ChangeTaskCompleteStatusCommandHandler : ICommandHandler<ChangeTaskCompleteStatusCommand>
{
    private readonly IUnitOfWork _unitOfWork;

    public ChangeTaskCompleteStatusCommandHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }
    public async Task<Result> Handle(ChangeTaskCompleteStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        task.ChangeCompleteState(request.Dto.IsComplete);
        ...
        return Result.Ok()
            .WithSuccess(new CompleteSuccess(nameof(Task)));
    }
}
